...
import { publishHistoryEntry } from "./actions";
...
type HistoryEntry = { id: string; title: string; body: string; published_at: Date };
...
export default async function UpdateHistoryPage({ searchParams }: { searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  const entries = await sql<HistoryEntry[]>`select id, title, body, published_at from update_history_entries order by published_at desc, id desc`;
    ...
    <div className="live-feedback-inner">
      ...
      <div className="live-history-list">
        {entries.length ? entries.map(entry => <article className="panel live-history-entry" key={entry.id}>
          <time dateTime={entry.published_at.toISOString()}>{entry.published_at.toLocaleDateString("en-CA", { timeZone: "America/Vancouver", year: "numeric", month: "long", day: "numeric" })}</time>
          <h2>{entry.title}</h2>
