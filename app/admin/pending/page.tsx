    ...
    <section className="section-block">
      {pending.length ? <div className="question-list">{pending.map((item) => <article className="question-card" key={item.id}>
        ...
        <ComposerDialog buttonLabel="Review" title="Review submission" className="secondary"><form action={reviewQuestionAction} className="composer-form">
          ...
          <div className="card-actions"><PendingButton name="intent" value="reject" className="danger" formNoValidate pendingText="Rejecting…">Reject</PendingButton><PendingButton name="intent" value="save" className="secondary" pendingText="Saving…">Save</PendingButton><PendingButton name="intent" value="approve" className="primary" pendingText="Approving…">Approve</PendingButton></div>
        </form></ComposerDialog>
      </article>)}</div> : <div className="empty-state"><p>Nothing to review. New contributor submissions show up here.</p><a href="/admin/approved" className="secondary">See what’s scheduled →</a></div>}
    </section>
  </AdminShell>;
}
