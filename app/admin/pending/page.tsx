  ...
  return (
    <AdminShell page="pending" username={session.username} title="Pending" description="Contributor submissions waiting for review. Edit anything, then approve, save, or reject." notice={params} actions={<span className="count-badge large">{pending.length} waiting</span>}>
      <section className="section-block">
        {pending.length ? <div className="question-list">{pending.map((item) => <form action={reviewQuestionAction} className="question-card" key={item.id}><input type="hidden" name="id" value={item.id} /><div className="question-meta"><span>Submitted {relativeDate(item.created_at)}</span><span className="status pending">{item.question_type === "event" ? "Event" : "Announcement"}</span></div><AdminEntryFields idPrefix={item.id} announcementMinimumDate={announcementMinimumDate} eventMinimumDate={eventMinimumDate} initialType={item.question_type} initialTitle={item.event_title || ""} initialDate={scheduledDateValue(item.scheduled_date) || ""} initialAnnouncement={item.question} initialDaysEarly={Number(item.days_early) || 0} announcementTemplate={templates.announcementTemplate} eventTemplate={templates.eventTemplate} />{item.contributor_note && <p className="review-note"><strong>Note:</strong> {item.contributor_note}</p>}<div className="card-actions"><PendingButton name="intent" value="reject" className="danger" formNoValidate pendingText="Rejecting…">Reject</PendingButton><PendingButton name="intent" value="save" className="secondary" pendingText="Saving…">Save</PendingButton><PendingButton name="intent" value="approve" className="primary" pendingText="Approving…">Approve</PendingButton></div></form>)}</div> : <div className="empty-state"><p>Nothing to review. New contributor submissions show up here.</p><a href="/admin/approved" className="secondary">See what’s scheduled →</a></div>}
      </section>
    </AdminShell>
  );
}
