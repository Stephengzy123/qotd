...
export async function quickAnnounceAction(_previous: Result, form: FormData): Promise<Result> {
  ...
  if (replyToId && !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(replyToId)) return { error: "Choose a valid announcement to reply to." };
  if (!body.trim() || body.length > 2000) return { error: "Write a message of 1–2,000 characters." };
  const sql = await dbReady();
  if (replyToId) {
    const replyRows = await sql`select id from dispatches where id = ${replyToId} and success = true and hidden_from_live = false`;
    if (!replyRows.length) return { error: "That announcement is no longer available to reply to." };
  }
  ...
  const rows = await sql`insert into dispatches (id, mode, message, success, question_type, destination, sender_name, sender_avatar_url, reply_to_dispatch_id)
    values (${requestId}, 'manual_selected', ${message}, ${destination === "live"}, 'announcement', ${destination}, ${nickname}, ${avatar}, ${replyToId || null})
  ...
  await logEvent({ action: "quick_announcement", actor: session.username, role: session.role, success: !error, details: { dispatchId: requestId, nickname, destination, replyToId: replyToId || null, ping: false, destinations: results, responseStatus: status } });
