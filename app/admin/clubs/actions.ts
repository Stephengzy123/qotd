...
import { CLUB_POST_MAX_LENGTH, createClub, deleteClub, getClub, postClubMessage, removeMembership, renameClub, reviewClubPost, saveClubWebhook, setMembership, type ClubRole } from "@/lib/clubs";
...
export async function postAsClubAction(formData: FormData) {
  ...
  const result = await postClubMessage(club, { accountId: session.accountId ?? null, username: session.username, role: session.role }, message);
  refresh(id);
...
export async function setMemberRoleAction(formData: FormData) {
  ...
  const role: ClubRole = formData.get("clubRole") === "assistant" ? "assistant" : "leader";
  ...
  refresh(clubId);
  redirect(messageUrl(path, "ok", role === "leader" ? "Now a club leader." : "Now a club assistant."));
}

export async function reviewClubPostAdminAction(formData: FormData) {
  const session = await requireRole("admin");
  const clubId = String(formData.get("clubId") || "");
  const postId = String(formData.get("postId") || "");
  const decision = formData.get("decision") === "reject" ? "reject" : "approve";
  const path = `/admin/clubs/${clubId}`;
  if (!UUID.test(clubId) || !UUID.test(postId)) await fail("/admin/clubs", session, "approve_club_post", "Invalid post.", { clubId, postId });
  const club = (await getClub(clubId)) ?? await fail("/admin/clubs", session, "approve_club_post", "That club no longer exists.", { clubId });
  const result = await reviewClubPost(club, postId, { accountId: session.accountId ?? null, username: session.username, role: session.role }, decision);
  refresh(clubId);
  redirect(result.error !== undefined ? messageUrl(path, "error", result.error) : messageUrl(path, "ok", decision === "approve" ? "Approved and posted." : "Post rejected."));
}
