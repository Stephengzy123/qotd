...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ ok?: string; error?: string }> }) {
  ...
  return (
    <main className="app-shell">
      ...
      <header className="topbar"><strong>Announcement admin</strong><nav><a href="#inbox">Pending</a><a href="#approved">Approved</a><a href="#delivery">Settings</a><a href="#accounts">Accounts</a><a href="#activity">Activity</a><a href="/admin/logs">Logs</a></nav><div className="account"><span>{session.username}</span><form action={logoutAction}><PendingButton className="text-button" pendingText="Signing out…">Sign out</PendingButton></form></div></header>
        ...
        <div className="activity-list">
          ...
          {accounts.map((account) => <div key={account.id} className="account-row"><span className="activity-dot success" /><div><strong>{account.username}</strong><p>{account.role}{account.created_by ? ` · created by ${account.created_by}` : ""} · added {relativeDate(account.created_at)}</p>
            <form action={updateAccountAction} className="account-manage"><input type="hidden" name="id" value={account.id} />
              ...
              <label className="sr-only" htmlFor={`password-${account.id}`}>New password</label><input id={`password-${account.id}`} name="password" type="password" autoComplete="new-password" minLength={8} maxLength={128} placeholder="New password (optional)" />
