...
export default async function LogsPage({ searchParams }: { searchParams: Promise<Filters> }) {
  ...
  return (
    <main className="app-shell">
      <header className="topbar"><strong>Announcement admin</strong><nav><a href="/admin#inbox">Pending</a><a href="/admin#approved">Approved</a><a href="/admin#delivery">Settings</a><a href="/admin#accounts">Accounts</a><a href="/admin/logs" aria-current="page">Logs</a></nav><div className="account"><span>{session.username}</span><form action={logoutAction}><PendingButton className="text-button" pendingText="Signing out…">Sign out</PendingButton></form></div></header>
