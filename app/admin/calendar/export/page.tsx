...
export default async function CalendarExportPage() {
  ...
  return <AdminShell page="calendar" username={session.username} title="Calendar export lab" description="Admin-only testing for shared calendars and personal timetable subscriptions." actions={<a className="secondary" href="/admin/calendar">Back to calendar</a>}>
    <section className="section-block"><h2>My timetable</h2><p className="hint">Enter your classes for A–H. The imported rotation determines their order each day; school templates supply the times, including Wednesdays and Flex Days. Activities and available lunch menus are included.</p><p className="hint"><strong>Personal planning only:</strong> this is not the official school calendar. It reflects normal school days and imported block rotations, so check official announcements for exceptions. It supports one semester at a time—replace the classes when your semester changes; it cannot hold both semesters at once.</p><p className="hint">Want to use this in the public calendar? <a href="/live/timetable">Open My timetable</a> in this browser to connect the same subscription link and classes. Once connected, edits in either editor update the same timetable.</p><PersonalTimetableExport storageKey={`personal-timetable:${session.accountId || session.username}`} /></section>
