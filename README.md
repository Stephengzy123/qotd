...
Event announcements have separate publish and occurrence dates. An optional end date supports multi-day events; leaving it blank creates a single-day event. Multi-day events render as continuous bars across the month calendar, splitting only when they continue onto a new week. Approved and sent events with an occurrence date appear on the admin calendar. The Calendar page also provides a guided **Complete missing dates** flow for stepping through older sent events, remembers skipped events for the rest of that pass, and includes a list for correcting individual dates.
