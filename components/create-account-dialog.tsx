...
const ACCOUNT_TYPES: { value: Role; label: string; description: string; badge: string }[] = [
  ...
  { value: "club_leader", label: "Club manager", description: "Manages a club and posts directly to its assigned Discord channels.", badge: "mode-leader" },
...
export function CreateAccountDialog({ clubs }: { clubs: ClubOption[] }) {
  ...
  function show() {
    setStep("type"); setRole(null); setUsername(""); setClubName(""); setClub(clubs[0]?.id || "new");
    setOpen(true);
  ...
  return <>
    ...
    <dialog ref={dialog} className="composer-dialog account-dialog" aria-labelledby={titleId} onClose={() => setOpen(false)} onCancel={(event) => { event.preventDefault(); close(); }}>
      <div className="composer-dialog-header">
        <div><h2 id={titleId}>Create an account</h2><p className="hint">Step {step === "type" ? 1 : 2} of 2 · {step === "type" ? "Choose the account type" : role === "club_leader" ? "Username and club" : "Choose a username"}</p></div>
