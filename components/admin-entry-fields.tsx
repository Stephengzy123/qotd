...
export function AdminEntryFields({ idPrefix, announcementMinimumDate, eventMinimumDate, initialType = "announcement", initialTitle = "", initialDate, initialAnnouncement = "", initialDaysEarly = 0, announcementTemplate, eventTemplate }: AdminEntryFieldsProps) {
  ...
  return (
    <div className="entry-fields">
      ...
      <div><label htmlFor={`${idPrefix}-announcement`}>Announcement</label><textarea id={`${idPrefix}-announcement`} name="announcement" value={announcement} onChange={(event) => setAnnouncement(event.target.value)} rows={6} minLength={8} maxLength={1500} required /></div>
      {announcementTemplate && eventTemplate && <AnnouncementPreview type={type} announcement={announcement} eventTitle={eventTitle} scheduledDate={scheduledDate} announcementTemplate={announcementTemplate} eventTemplate={eventTemplate} />}
    </div>
  );
}
