...
export function AdminEntryFields({ idPrefix, announcementMinimumDate, eventMinimumDate, initialType = "announcement", initialTitle = "", initialDate, initialOccurrenceDate, initialAnnouncement = "", initialDaysEarly = 0, announcementTemplate, eventTemplate }: AdminEntryFieldsProps) {
  ...
  return (
    <div className="entry-fields">
      ...
      {type === "event" && <div><label htmlFor={`${idPrefix}-occurrence-date`}>Event occurrence date</label><input id={`${idPrefix}-occurrence-date`} name="occurrenceDate" type="date" min={scheduledDate || eventMinimumDate} value={occurrenceDate} onChange={(event) => setOccurrenceDate(event.target.value)} required /><p className="hint">The event appears on the admin calendar on this date.</p></div>}
