...
export function CalendarDayView({ date, periods, events, onSelect, personalized = false, editHref }: { date: string; periods: DailyPeriod[]; events: AdminCalendarEvent[]; onSelect: (event: AdminCalendarEvent) => void; personalized?: boolean; editHref?: string }) {
  ...
  return <div className="calendar-day-view">
    ...
    <div className="calendar-day-note"><span>{personalized ? "Your saved classes · Vancouver time" : "Times shown in Vancouver time"}</span><a href={editHref || (personalized ? "/admin/calendar/export" : "/admin/timetable")}>{personalized ? "Edit my timetable" : "Set up my timetable"}</a></div>
