...
type RichTextEditorProps = { id: string; label: string; value: string; onChange: (markdown: string) => void };

// A contenteditable surface that keeps the Markdown string as its real value.
export function RichTextEditor({ id, label, value, onChange }: RichTextEditorProps) {
  ...
  function emit() {
    const editor = editorRef.current;
    if (!editor || editor.innerHTML === lastHtml.current) return;
    lastHtml.current = editor.innerHTML;
  ...
  return (
    <>
      ...
      <div ref={editorRef} id={id} className="rich-editor" contentEditable suppressContentEditableWarning role="textbox" aria-label={label} aria-multiline="true" onInput={emit} onBlur={emit} onKeyDown={onKeyDown} onPaste={onPaste} />
    </>
  );
}
