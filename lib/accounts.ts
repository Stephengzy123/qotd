...
export async function findAccount(username: string) {
  const sql = await dbReady();
  const rows = await sql<{ id: string; username: string; role: Role; password_hash: string }[]>`
...
export async function createAccount(username: string, password: string, role: Role, createdBy: string): Promise<{ error: string; id?: undefined } | { id: string; error?: undefined }> {
  ...
  if (password.length < 12 || Buffer.byteLength(password, "utf8") > 72) return { error: "Passwords must be at least 12 characters and at most 72 bytes." };
...
export async function updateAccount(id: string, role: Role, password: string | null): Promise<{ error: string; username?: undefined } | { username: string; error?: undefined }> {
  if (password !== null && (password.length < 12 || Buffer.byteLength(password, "utf8") > 72)) return { error: "Passwords must be at least 12 characters and at most 72 bytes." };
  const sql = await dbReady();
