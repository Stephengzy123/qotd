...
export function subscriptionCalendar(events: AdminCalendarEvent[], feed: CalendarFeed, origin: string, now = new Date()) {
  const selected = events.filter(event => {
    const rotation = event.kind === "imported" && /\bday\s*[12]\b|\b[ABCDEFGH]{4}\b|block\s*rotation/i.test(event.title);
    return feed === "lunch" ? event.kind === "lunch" : feed === "rotations" ? rotation : event.kind !== "lunch" && !rotation;
  });
  const lines = ["BEGIN:VCALENDAR", "VERSION:2.0", "PRODID:-//Announcement Bot//School Calendar//EN", "CALSCALE:GREGORIAN", "METHOD:PUBLISH", `X-WR-CALNAME:${calendarFeeds[feed]}`];
