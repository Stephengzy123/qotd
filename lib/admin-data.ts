...
export type Announcement = { id: string; question: string; contributor_note: string | null; status: string; created_at: Date; scheduled_date: string | Date | null; question_type: "announcement" | "event"; event_title: string | null; days_early: number };
...
export async function listAnnouncements(status: "pending" | "approved") {
  const sql = await dbReady();
  return sql<Announcement[]>`select id, question, contributor_note, status, created_at, scheduled_date, question_type, event_title, days_early from questions where status = ${status} order by scheduled_date asc nulls last, created_at asc`;
}
