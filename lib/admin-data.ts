...
export async function listAnnouncements(status: "pending" | "approved") {
  const sql = await dbReady();
  return sql<Announcement[]>`select delivery_destination, remove_pings, discord_webhook_ids, send_schedule_mode, send_at, id, question, contributor_note, status, created_at, scheduled_date, question_type, event_title, event_occurrence_date, event_occurrence_end_date, days_early from questions where status = ${status} order by case when send_schedule_mode = 'exact' then send_at else scheduled_date::timestamptz end asc nulls last, created_at asc`;
}
